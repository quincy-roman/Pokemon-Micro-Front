...
export class APIPokemon {
    ...
    constructor(pokemonId:number,species:string, nickname:string, OT:number,currentTrainer:number,typeOne:string, typeTwo:string, ability:string,
        level:number, status:string, maxHp:number,  currentHp:number, atk:number, def:number, spAtk:number, spDef:number,  spd:number, rarity:string,isFainted:boolean,  inBattle:boolean, sprite:string)
        {
